...
import { ensureDir, writeJsonFile } from '../utils/fs-operations'
...
/**
 * Start a lightweight HTTP proxy on a random available port.
 * Intercepts POST /v2/* requests, captures all request bodies,
 * and forwards everything transparently to the real CodeBuddy API backend.
 */
...
function prepareTrace(
  ...
  writeJsonFile(`${sessionDir}/${timestamp}-request.json`, {
    meta: {
      sessionId,
      timestamp,
 ...
 * Heuristic: the body with messages array that contains a "Hello" or
 * the user's actual query (not a system prompt for memory selection).
 */
export function findMainChatBody(bodies: unknown[]): Record<string, unknown> | null {
  // Find the body that looks like a main chat request:
  // has messages with at least one user message, and has tools definitions
  for (const body of bodies) {
    ...
    // Has tools
    const hasTools = Array.isArray(b['tools']) && ((b['tools'] as Array<Record<string, unknown>>).length > 0)
    // Not a memory selection request (which typically has only 1-2 messages and no tools)
