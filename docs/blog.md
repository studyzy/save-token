# AI Coding Agent Token 成本优化指南：从使用习惯到代码图谱与多 Agent 协作

> 适用：CodeBuddy 这类 AI Coding Agent。
>
...
flowchart TD
    ...
    subgraph A["Agent / CLI"]
        direction TB
        S1["取系统提示词"] --> S2["拼历史消息或摘要"] --> S3["附 Skill / Tool 定义"] --> S4["读取代码、文档、检索结果"] --> S5["组装完整请求"]
    end

    A --> L["LLM API"] --> O["输出答案 / 发起工具调用"] --> N["结果再被塞回下一轮上下文"]
```
...
```
> **说明**：RTK 官方目前还没有支持CodeBuddy，所以使用 [studyzy Fork 版](https://github.com/studyzy/rtk)（已增加 CodeBuddy 支持）。
...
```mermaid
graph
    A["你的 Agent(CodeBuddy)"]
    A -->|"提示词 · 工具输出 · 日志 · RAG 结果 · 文件"| B

    subgraph Headroom ["Headroom（本地代理）"]
        direction LR
        B["缓存对齐器<br/>CacheAligner"] --> C["内容路由器<br/>ContentRouter"]
        C --> D["上下文压缩与召回<br/>CCR"]
        C --> E["智能粉碎器<br/>SmartCrusher (JSON)"]
        C --> F["代码压缩器<br/>CodeCompressor (AST)"]
        C --> G["通用压缩器<br/>Kompress-base (文本/HF)"]
        D --> Z["压缩后上下文"]
        E --> Z["压缩后上下文"]
        F --> Z["压缩后上下文"]
        G --> Z["压缩后上下文"]
...
headroom 不只是独立压缩器，它还集成了多个工具以覆盖更完整的上下文优化链路：

- **RTK**：内置终端输出压缩
- **lean-ctx**：整合 context engineering layer，在读取侧做智能筛选
- **Serena**：语义代码搜索，减少盲搜 Token 浪费
- **[codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp)**：代码知识图谱，让 Agent 先查图再读文件

这些集成意味着你不需要单独装多个工具——headroom 一条命令就把压缩、检索、记忆三层打通了。

## 4.4 lean-ctx：context engineering layer，不只是压缩

lean-ctx 和前面三个工具的思路不同：**它不只是在"内容已经进入上下文之后"做压缩，而是在"内容进入上下文之前"就做决策**。

它的核心定位是 context engineering layer —— 决定 Agent 读什么、怎么读、记住什么、以及证明省了什么。
...
graph
    A["你的 Agent（CodeBuddy）"]
    A -->|"每次读取、搜索、shell 调用"| B

    subgraph leanctx ["lean-ctx（Context Engineering Layer）"]
        direction TB
        B["智能读取<br/>10 种读取模式"] --> C["跨会话记忆<br/>持久化知识存储"]
        C --> D["安全防护<br/>PathJail · 脱敏 · 预算"]
        D --> E["签名账本<br/>Ed25519 可验证节省证明"]
    end

    E --> F["大模型供应商<br/>（DeepSeek / GLM / OpenAI …）"]
```

这张图解释了三件事：

1. **lean-ctx 是读取层，不只是压缩器**——Agent 不直接读文件、执行命令，而是先经过 lean-ctx。每次读取、搜索、shell 调用都先被 lean-ctx 处理。
2. **五种读取模式，按意图自动选择**——同一个文件，探索用 `map`（只返回导出和签名），编码用 `full`（完整内容），编辑后用 `diff`（只看变化）。不需要手动指定模式，lean-ctx 根据任务自动判断。
3. **跨会话记忆**——读取过的内容被缓存，重读同一个文件只需要 ~13 个 token，与文件大小无关。

### 实际效果

同一个文件 `src/lib/auth.ts`（约 2000 个 token），三种读取模式：

| 模式 | 场景 | 原始 | 输出 | 节省 |
|------|------|------|------|------|
| `map` | 探索代码库 | 4,200 | 180 | **96%** |
| `signatures` | 调用 API | 4,200 | 420 | **90%** |
| `full` | 编辑代码 | 4,200 | 4,200 | 0%（需要全部） |
| **缓存重读** | 再次读同一文件 | — | 13 | **~99.7%** |

按操作类型的典型压缩率：

| 操作类型 | 原始 tokens | lean-ctx 后 | 节省 |
|---------|-----------|------------|------|
| 读取源文件（map） | 3,000 | 750 | **-75%** |
| 读取源文件（auto） | 3,000 | 540 | **-82%** |
| Shell: git diff | 2,000 | 500 | **-75%** |
| Shell: cargo test | 2,500 | 300 | **-88%** |
| 代码搜索 | 5,000 | 1,000 | **-80%** |
| 目录扫描 | 2,000 | 400 | **-80%** |
| API 响应 | 3,000 | 600 | **-80%** |

典型开发会话：~10,500 → ~2,090 tokens，**整体节省约 80%**。

### 和 headroom 的区别

headroom 是在"请求发送给模型之前"做可逆压缩。lean-ctx 是在"内容被读取时"就做筛选和压缩，同时还做跨会话记忆和安全防护：

| | lean-ctx | headroom |
|--|----------|----------|
| 压缩时机 | 读取时（源头） | 发送前（传输线） |
| 跨会话记忆 | 内置，缓存重读 ~13 tokens | 无 |
| 安全防护 | PathJail、脱敏、预��控制 | 无 |
| 节省证明 | Ed25519 签名账本 | 无 |
| 典型节省 | 60–90% | 47–92% |

两者方向不同，可以叠加使用。lean-ctx 在源头筛选，headroom 在传输线压缩。
...
安装后运行 `lean-ctx setup`，lean-ctx 会自动检测已安装的 AI 工具并配置好钩子和 MCP 服务器。支持 30+ 种 AI 工具，包括 CodeBuddy、Cursor、Claude Code、GitHub Copilot 等。
...
```bash
lean-ctx stats       # 查看节省统计
lean-ctx gain        # 查看累计节省

# 手动读取（Agent 自动调用，通常不需要手动执行）
lean-ctx read src/lib/auth.ts -m map       # map 模式：只返回签名
lean-ctx read src/lib/auth.ts -m full      # full 模式：完整内容
lean-ctx search "authenticate" src/        # 代码搜索
...
| lean-ctx | 所有读取内容 + 跨会话记忆 + 安全防护 | 60–90% | `brew install lean-ctx` + `lean-ctx setup` |

最低成本起点：先装 RTK + Caveman，五分钟，立即见效。有时间再评估 lean-ctx 和 headroom。
...
CodeGraph 是另一个方向的工具，基于 MCP Server + 持久化图数据库（Neo4j/KuzuDB），对 7 个真实开源仓库做了完整 benchmark：
...
除了 Graphify 和 CodeGraph，还有 **[codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp)**（2026 年 2 月发布，70+ 贡献者）。同样是通过 MCP Server 将代码仓库索引为知识图谱，但有几个差异化特点：

- **纯 C 语言单二进制**，零运行时依赖，部署极简
- **Hybrid LSP**：内置 C 实现的类型解析引擎，无需启动独立语言服务器
- **158 种语言**：通过 tree-sitter 覆盖，10 种语言有深度类型解析
- **极致性能**：Linux 内核（2800 万行代码）3 分钟索引完成，查询 < 1ms
- **Cypher 图查询**：支持类图查询语法，灵活度更高
- **团队共享**：压缩图文件 `.codebase-memory/graph.db.zst` 可提交到仓库，队友直接复用

与 Graphify 相比，codebase-memory-mcp 更像 CodeGraph 的竞品——都是 MCP 服务器形态、持久化图数据库。选型的核心判断维度是语言覆盖和团队协作需求。
...
更低成本
=
更少重复上下文（RTK、Caveman、lean-ctx、headroom、会话管理）
...
- 评估 lean-ctx 或 headroom，控制更大规模的上下文输入
...
[12] lean-ctx 官网  
https://leanctx.com/zh/

[13] codebase-memory-mcp GitHub  
